...
SkillType NaoBehavior::play()
{
	...
	for(int i = WO_OPPONENT1; i <= WO_OPPONENT11; ++i) {
                ...
                if (opponent->validPosition == true) {
                    ...
                    temp.setZ(0);
                    if (abs(me.getAngleBetweenPoints(ball, temp)) < 90.0) {
	...
	if(closestObjDistance >= 5)
		return kickBall(KICK_DRIBBLE, VecPosition(HALF_FIELD_X, 0, 0));
	else
	{
		VecPosition ballpos = worldModel->g2l(ball);
		VecPosition me2opp = closestObjPos - me;
		VecPosition me2ball = ballpos - me;
		VecPosition opp2ball = ballpos - closestObjPos;
		SIM::AngDeg angle1 = abs(VecPosition(0, 0, 0).getAngleBetweenPoints(me2opp, me2ball));
		SIM::AngDeg angle2 = abs(VecPosition(0, 0, 0).getAngleBetweenPoints(me2opp, opp2ball));
		SIM::AngDeg angle = angle2 - angle1;
		return goToTargetRelative( ballpos , angle);
